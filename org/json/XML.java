...
public class XML
{
    ...
    return new Iterable<Integer>()
      {
        public Iterator<Integer> iterator() {
          return new Iterator<Integer>() {
              ...
              public boolean hasNext() { return (this.nextIndex < this.length); }
  ...
  public static String escape(String string) {
    ...
    for (Iterator<Integer> iterator = codePointIterator(string).iterator(); iterator.hasNext(); ) { int cp = ((Integer)iterator.next()).intValue();
      switch (cp) {
        case 38:
          sb.append("&amp;");
          continue;
  ...
  private static boolean mustEscape(int cp) { return ((Character.isISOControl(cp) && cp != 9 && cp != 10 && cp != 13) || ((cp < 32 || cp > 55295) && (cp < 57344 || cp > 65533) && (cp < 65536 || cp > 1114111))); }
  ...
  private static boolean parse(XMLTokener x, JSONObject context, String name, boolean keepStrings) throws JSONException {
    ...
    while (true) {
      ...
      if (token instanceof String) {
        ...
        jsonobject.accumulate(string, ""); continue;
      } 
      break;
    } 
    if (token == SLASH) {
  ...
  public static JSONObject toJSONObject(String string) throws JSONException { return toJSONObject(string, false); }
